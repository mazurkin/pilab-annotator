...
class MainWindow(QtGui.QMainWindow):
    ...
    def imageMousePressEvent(self, event):
        ...
        if self.ui.image.pixmap() and currentTool == "point":
            ...
            if modes["rectangle"] == "draw":
                ...
                self.dragIsActive = False
                for (i, j, k, m) in rectangles[currentIndex]:
                    if ((abs(x - i) <= 2 or abs(x - i - k) <= 2) and -2 <= y - j <= m + 2) or (
                        (abs(y - j) <= 2 or abs(y - j - m) <= 2) and -2 <= x - i <= k + 2):
                ...
                if self.resizeIsActive:
                    ...
                    if width != 0 and height != 0:
                        ...
                        if height < 0:
                            height = abs(height)
                            y = event.pos().y()
                        command = CommandAddRect(currentIndex, (x, y, width, height),
                                                 "Add Rectangle @(%d-%d-%d-%d)" % (x, y, width, height))
                        self.undoStacks[currentIndex].push(command)
                        annotationChanged[currentIndex] = True
                    if self.drawingRectangle:
                        self.drawingRectangle = False
        ...
        if self.ui.zoomImage.pixmap():
            ...
            if pointsVisible and len(zoomPoints) > 0:
                for (i, j) in zoomPoints:
                    if useSmartColor:
                        r, g, b = getSmartColor(
                            currentImage.getpixel(((i + self.left) / zoomAmount, (j + self.up) / zoomAmount)))
            ...
            self.ui.zoomImage.paint.drawLine(0, self.ui.zoomImage.height() / 2, self.ui.zoomImage.width(),
                                             self.ui.zoomImage.height() / 2)
            self.ui.zoomImage.paint.drawLine(self.ui.zoomImage.width() / 2, 0, self.ui.zoomImage.width() / 2,
                                             self.ui.zoomImage.height())
            self.ui.zoomImage.paint.drawEllipse(self.ui.zoomImage.width() / 2 - 15, self.ui.zoomImage.height() / 2 - 15,
                                                30, 30)
            ...
            try:
                ...
                if len(imageFiles) > 0:
                    for imageFile in imageFiles:
                        objects = []
                        try:
                            annotation_file = os.path.join(path, os.path.splitext(imageFile)[0] + ".xml")
                            ...
                            point_objects = xmldoc.getElementsByTagName("point")
                            for point_object in point_objects:
                                x = int(point_object.getAttribute("x"))
                                y = int(point_object.getAttribute("y"))
                                pts.append((x, y))

                            rect_objects = xmldoc.getElementsByTagName("rect")
                            for rect_object in rect_objects:
                                x = int(rect_object.getAttribute("x"))
                                y = int(rect_object.getAttribute("y"))
                                w = int(rect_object.getAttribute("w"))
                                h = int(rect_object.getAttribute("h"))
                        ...
                        self.connect(self.undoStacks[-1], QtCore.SIGNAL("indexChanged(int)"), self.ui.zoomImage,
                                     QtCore.SLOT("repaint()"))
    ...
    def saveAnnotations(self):
        ...
        annotation.setAttribute("filename", filename)
        doc.appendChild(annotation)

        point_objects = doc.createElement("points")
        point_objects.setAttribute("count", str(len(points[currentIndex])))
        annotation.appendChild(point_objects)

        for (x, y) in points[currentIndex]:
            point_object = doc.createElement("point")
            point_object.setAttribute("x", str(x))
            point_object.setAttribute("y", str(y))
            point_objects.appendChild(point_object)

        rect_objects = doc.createElement("rects")
        rect_objects.setAttribute("count", str(len(rectangles[currentIndex])))
        annotation.appendChild(rect_objects)

        for (x, y, w, h) in rectangles[currentIndex]:
            rect_object = doc.createElement("rect")
            rect_object.setAttribute("x", str(x))
            rect_object.setAttribute("y", str(y))
            rect_object.setAttribute("w", str(w))
            rect_object.setAttribute("h", str(h))
            rect_objects.appendChild(rect_object)
    ...
    def loadImage(self, path):
        ...
        self.zoomedPixmap = pixmap.scaled(self.ui.image.width() * zoomAmount, self.ui.image.height() * zoomAmount,
